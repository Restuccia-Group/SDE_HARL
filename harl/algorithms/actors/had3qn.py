...
class HAD3QN(OffPolicyBase):
    ...
    def get_actions(self, obs, epsilon_greedy):
        ...
        if np.random.random() < self.epsilon and epsilon_greedy:
            ...
            logits, bpp_loss, aux_loss = self.actor(obs)
            actions = logits.argmax(dim=-1, keepdim=True)
        return actions, logits, bpp_loss, aux_loss
    ...
    def get_target_actions(self, obs):
        ...
        """
        obs = check(obs).to(**self.tpdv)
        logits, bpp_loss, aux_loss = self.target_actor(obs)
    ...
    def train_values(self, obs, actions):
        ...
        values = torch.gather(logits, dim=1, index=actions)
        return values, logits, bpp_loss, aux_loss
