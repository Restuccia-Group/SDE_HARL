...
class HASAC(OffPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if act_space.__class__.__name__ == "Box":
            self.actor = SquashedGaussianPolicy(args, obs_space, act_space, num_agents, device)
        else:
            self.actor = StochasticMlpPolicy(args, obs_space, act_space, num_agents, device)
    ...
    def get_actions(self, obs, available_actions=None, stochastic=True):
        ...
        if self.action_type == "Box":
            actions, _, bpp_loss, aux_loss = self.actor(obs, stochastic=stochastic, with_logprob=False)
        else:
            actions, bpp_loss, aux_loss = self.actor(obs, available_actions, stochastic)
    ...
    def get_actions_with_logprobs(self, obs, available_actions=None, stochastic=True):
        ...
        obs = check(obs).to(**self.tpdv)
        if self.action_type == "Box":
            actions, logp_actions, logits, bpp_loss, aux_loss = self.actor(
                obs, stochastic=stochastic, with_logprob=True
            )
        elif self.action_type == "Discrete":
            logits, bpp_loss, aux_loss = self.actor.get_logits(obs, available_actions)
        ...
        return actions, logp_actions, logits, bpp_loss, aux_loss
    ...
    def get_log_pi_withoutID(self, inp, out):
        pred = self.actor.get_log_pi_withoutID(inp, out)
        return pred
    

    def update_withoutID(self, inp, out):
        loss = self.actor.update_withoutID(inp, out)
        return loss
    
    def get_log_pi_withID(self, inp, out, add_id):
        pred = self.actor.get_log_pi_withID(inp, out, add_id)
        return pred

    def update_withID(self, inp, out, add_id):
        loss = self.actor.update_withID(inp, out, add_id)
        return loss
