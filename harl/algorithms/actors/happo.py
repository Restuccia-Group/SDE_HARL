...
class HAPPO(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        """
        super(HAPPO, self).__init__(args, obs_space, act_space, num_agents, device)
    ...
    def update(self, sample):
        ...
        if self.id_diversity and self.obs_diversity:
            intrinsic_rewards = self.beta_2 * pi_diverge + obs_diverge.mean()
            ...
            intrinsic_rewards = obs_diverge.mean()
        else:
        ...
        (policy_loss - dist_entropy * self.entropy_coef + 1e-5*bpp_loss).backward()  # add entropy term
        #print(bpp_loss)
