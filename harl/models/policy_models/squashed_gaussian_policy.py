...
class SquashedGaussianPolicy(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, num_agents, device=torch.device("cpu")):
        ...
        self.predict_withoutID = Predict_Network(obs_shape[0] + self.action_dim, hidden_sizes[0], obs_shape[0])
        self.predict_withID = Predict_Network_WithID(obs_shape[0] + self.action_dim + self.num_agents, hidden_sizes[0], obs_shape[0], self.num_agents)
        self.to(device)
    ...
    def forward(self, obs, stochastic=True, with_logprob=True):
        ...
        if self.feature_extractor is not None:
            ...
            x = obs
        net_out, bpp_loss, aux_loss = self.net(x)
        ...
        return pi_action, logp_pi, pi_distribution, bpp_loss, aux_loss
    ...
    def update_withID(self, inp, out, add_id):
        inp = check(inp).to(**self.tpdv)
        out = check(out).to(**self.tpdv)
        pred = self.predict_withID.update(inp, out, add_id)
        return pred
