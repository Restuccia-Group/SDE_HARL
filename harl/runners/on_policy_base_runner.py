...
class OnPolicyBaseRunner:
    ...
    def warmup(self):
        ...
        # replay buffer
        for agent_id in range(self.num_agents):
            self.actor_buffer[agent_id].next_obs[0] = obs[:, agent_id].copy()
    ...
    def collect(self, step):
        ...
        dis_actions = []
        for agent_id in range(self.num_agents):
            action, action_log_prob, rnn_state, dis_action = self.actor[agent_id].get_actions(
                self.actor_buffer[agent_id].next_obs[step],
            ...
            current_obs.append(self.actor_buffer[agent_id].next_obs[step])
            dis_actions.append(dis_action)
        # (n_agents, n_threads, dim) -> (n_threads, n_agents, dim)
        actions = np.array(action_collector).transpose(1, 0, 2)
        action_log_probs = np.array(action_log_prob_collector).transpose(1, 0, 2)
        rnn_states = np.array(rnn_state_collector).transpose(1, 0, 2, 3)
        current_obs = torch.stack([torch.from_numpy(obs) for obs in current_obs], dim=0).permute(1, 0, 2)
        dis_actions = torch.stack([dis for dis in dis_actions], dim=0).permute(1, 0, 2)
        ...
        return current_obs, values, actions, action_log_probs, dis_actions, rnn_states, rnn_states_critic

    def insert(self, data):
        """Insert data into buffer."""
        (
            current_obs, # (n_threads, n_agents, obs_dim)
            next_obs,  # (n_threads, n_agents, obs_dim)
            next_share_obs,  # (n_threads, n_agents, share_obs_dim)
            rewards,  # (n_threads, n_agents, 1)
            dones,  # (n_threads, n_agents)
            ...
            dis_actions, # (n_threads, n_agents, action_dim)
        ...
        # elif self.envs.action_space.__class__.__name__ == "MultiDiscrete":
        #     self.multidiscrete_action = True
        #     self.action_dim = action_space.nvec

        actions_onehot = F.one_hot(torch.from_numpy(actions), num_classes= self.envs.action_space[0].n)
        ...
        if self.state_type == "EP":
            ...
            self.critic_buffer.insert(
                next_share_obs, rnn_states_critic, values, rewards, masks, bad_masks
            )
