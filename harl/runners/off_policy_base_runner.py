...
class OffPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        self.obs_diversity = algo_args["algo"]["obs_aware"]
        self.id_diversity = algo_args["algo"]["id_aware"]
        self.beta = algo_args["algo"]["beta"]
        self.beta_1 = algo_args["algo"]["beta_1"]
        self.beta_2 = algo_args["algo"]["beta_2"]
        ...
        if self.args["algo"] == "hasac":
            ...
            for agent_id in range(self.num_agents):
                if (
                    ...
                    action, _, _ = self.actor[agent_id].get_actions(
                                obs[:, agent_id],
            ...
            actions = []
            for agent_id in range(self.num_agents):
                action, _, bpp_loss, aux_loss = self.actor[agent_id].get_actions(obs[:, agent_id], add_random)
                actions.append(_t2n(action))
    ...
    def eval(self, step):
        ...
        while True:
            ...
            if eval_episode >= self.algo_args["eval"]["eval_episodes"]:
                ...
                self.writter.add_scalar(
                    "Eval_score_rate", eval_score_cnt / eval_episode, step
                )
                break
