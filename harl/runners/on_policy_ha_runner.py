...
class OnPolicyHARunner(OnPolicyBaseRunner):
    ...
    def train(self):
        ...
        for agent_id in agent_order:
            ...
            # compute action log probs for the currently actor before update.
            old_actions_logprob, _, _, _, _ = self.actor[agent_id].evaluate_actions(
                self.actor_buffer[agent_id]
                .next_obs[:-1]
                .reshape(-1, *self.actor_buffer[agent_id].next_obs.shape[2:]),
                self.actor_buffer[agent_id]
                .rnn_states[0:1]
            ...
            new_actions_logprob, _, _, _, _ = self.actor[agent_id].evaluate_actions(
                self.actor_buffer[agent_id]
                .next_obs[:-1]
                .reshape(-1, *self.actor_buffer[agent_id].next_obs.shape[2:]),
                self.actor_buffer[agent_id]
