...
class OnPolicyActorBuffer:
    ...
    def __init__(self, args, obs_space, act_space, num_agents):
        ...
        self.current_obs = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, self.num_agents, *obs_shape),
            dtype=np.float32,
        )
        ...
        self.actions_onehot = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_agents, act_space.n), dtype=np.float32
        )
        # Buffer for action log probs of this actor.
        self.action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, act_shape), dtype=np.float32
        )
        self.dis_actions = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_agents, act_space.n), dtype=np.float32
        )
    ...
    def insert(
        ...
        self.current_obs[self.step + 1] = current_obs.numpy().copy()
        self.next_obs[self.step + 1] = next_obs.copy()
        self.next_obs_share[self.step + 1] = next_obs_share.copy()
        self.rnn_states[self.step + 1] = rnn_states.copy()
        self.actions[self.step] = actions.copy()
        self.actions_onehot[self.step] = actions_onehot.numpy().copy()
        self.action_log_probs[self.step] = action_log_probs.copy()
        self.dis_actions[self.step] = (dis_actions.cpu()).numpy().copy()
    ...
    def after_update(self):
        ...
        self.next_obs[0] = self.next_obs[-1].copy()
        self.next_obs_share[0] = self.next_obs_share[-1].copy()
    ...
    def feed_forward_generator_actor(
        ...
        current_obs = self.current_obs[:-1].reshape(-1, *self.current_obs.shape[2:])
        next_obs = self.next_obs[:-1].reshape(-1, *self.next_obs.shape[2:])
        next_obs_share = self.next_obs_share[:-1].reshape(-1, *self.next_obs_share.shape[2:])
        rnn_states = self.rnn_states[:-1].reshape(-1, *self.rnn_states.shape[2:])  # actually not used, just for consistency
        actions = self.actions.reshape(-1, self.actions.shape[-1])
        actions_onehot = self.actions_onehot.reshape(-1, *self.actions_onehot.shape[2:])
        if self.available_actions is not None:
        ...
        dis_actions = self.dis_actions.reshape(-1, *self.dis_actions.shape[2:])
        ...
        for indices in sampler:
            ...
            obs_current_batch = current_obs[indices]
            obs_next_batch = next_obs[indices]
            obs_next_share_batch = next_obs_share[indices]
            rnn_states_batch = rnn_states[indices]
            actions_batch = actions[indices]
            actions_onehot_batch = actions_onehot[indices]
            if self.available_actions is not None:
            ...
            dis_actions_batch = dis_actions[indices]
            ...
            if self.factor is None:
                yield obs_current_batch, obs_next_batch, obs_next_share_batch, rnn_states_batch, actions_batch, actions_onehot_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, dis_actions_batch, adv_targ, available_actions_batch
            else:
                factor_batch = factor[indices]
                yield obs_current_batch, obs_next_batch, obs_next_share_batch, rnn_states_batch, actions_batch, actions_onehot_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, dis_actions_batch, adv_targ, available_actions_batch, factor_batch
